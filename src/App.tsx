...
import { appBuildInfo, type AppBuildInfo } from './buildInfo'
...
const MAX_BROWSER_RECORDING_SECONDS = MAX_AUDIO_DURATION_SECONDS
...
const formatAudioSeekDebug = (entry: AudioSeekDebugEntry) => {
  const fmt = (value: number | null) => (value === null ? '-' : (Number.isFinite(value) ? value.toFixed(2) : String(value)))
  return `mime=${entry.mimeType ?? '-'} ext=${entry.extension ?? '-'} native=${fmt(entry.nativeDuration)} saved=${fmt(entry.savedDuration)} before=${fmt(entry.before)} next=${fmt(entry.nextTime)} afterImm=${fmt(entry.afterImmediate)} afterDelay=${fmt(entry.afterDelay)} rs=${entry.readyState} ns=${entry.networkState}${entry.ignored ? ' [IGNORED?]' : ''}`
}
...
const [availableBuildInfo, setAvailableBuildInfo] = useState<AppBuildInfo | null>(null)
...
const currentBuildLabel = `${appBuildInfo.version} (${appBuildInfo.commitSha})`
const isComposeDirty = composeText.trim().length > 0 || !!recordedBlob || !!uploadedAudio
const hasPendingCommentDraft = Object.values(commentInputMap).some((value) => value.trim().length > 0)
const hasPendingVoiceReply = Object.keys(voiceReplyBlobByPostId).length > 0 || Object.keys(voiceReplyPreviewUrlByPostId).length > 0
const hasPendingProfileEdit = isEditingProfile && (
  profileEditForm.display_name !== (profile?.display_name ?? '') ||
  profileEditForm.username !== (profile?.username ?? '') ||
  profileEditForm.bio !== (profile?.bio ?? '') ||
  profileEditForm.avatar_url !== (profile?.avatar_url ?? '') ||
  !!selectedAvatarFile
)
const isBusyPosting = isPosting || Object.values(commentPostingMap).some(Boolean) || Object.values(voiceCommentPostingMap).some(Boolean) || isSavingProfile
const isUpdateUnsafe = isRecording || !!voiceReplyRecordingPostId || isBusyPosting || isComposeDirty || hasPendingCommentDraft || hasPendingVoiceReply || hasPendingProfileEdit
const updateButtonLabel = isUpdateUnsafe ? '入力や録音を完了してから更新' : '更新する'

useEffect(() => {
  if (typeof navigator === 'undefined' || !('serviceWorker' in navigator)) return
  navigator.serviceWorker.getRegistrations().then((registrations) => {
    registrations.forEach((registration) => {
      registration.unregister().catch((error) => console.warn('Failed to unregister stale service worker', error))
    })
  }).catch((error) => console.warn('Failed to inspect service workers', error))
}, [])
...
useEffect(() => {
  ...
  const checkBuildInfo = async () => {
    try {
      const response = await fetch(BUILD_INFO_URL, { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } })
      if (!response.ok) return
      const remoteBuildInfo = await response.json() as Partial<AppBuildInfo>
      if (cancelled || !remoteBuildInfo.buildId) return
      if (remoteBuildInfo.buildId !== appBuildInfo.buildId) {
        setAvailableBuildInfo({
          version: remoteBuildInfo.version || 'unknown',
          buildId: remoteBuildInfo.buildId,
          commitSha: remoteBuildInfo.commitSha || 'unknown',
          builtAt: remoteBuildInfo.builtAt || ''
        })
      }
    } catch (error) {
      console.warn('Failed to check Friendcast build info', error)
    }
  }

  void checkBuildInfo()
  const intervalId = window.setInterval(() => { void checkBuildInfo() }, BUILD_INFO_CHECK_INTERVAL_MS)
  const handleVisibilityChange = () => {
    if (document.visibilityState === 'visible') void checkBuildInfo()
  }
  window.addEventListener('focus', checkBuildInfo)
  document.addEventListener('visibilitychange', handleVisibilityChange)

  return () => {
    cancelled = true
    window.clearInterval(intervalId)
    window.removeEventListener('focus', checkBuildInfo)
    document.removeEventListener('visibilitychange', handleVisibilityChange)
  }
}, [])

const handleReloadToAvailableBuild = () => {
  if (isUpdateUnsafe) {
    setBuildUpdateMessage('録音・入力・投稿中の内容を完了または保存してから更新してください。')
    return
  }
  setBuildUpdateMessage('')
...
const handleUploadAudioChange = async (event: ChangeEvent<HTMLInputElement>) => {
  ...
  if (file.size > MAX_UPLOAD_AUDIO_SIZE_BYTES) {
    clearUploadedAudio()
    setUploadAudioError('音声ファイルは30MB以内にしてください。')
    return
  }
  ...
  try {
    ...
    if (durationSeconds > MAX_UPLOAD_AUDIO_DURATION_SECONDS) {
      ...
      clearUploadedAudio()
      setUploadAudioError('音声ファイルは20分以内にしてください。')
      return
    }
    clearRecordedAudio()
    setUploadedAudio((current) => {
    ...
    URL.revokeObjectURL(previewUrl)
    clearUploadedAudio()
    setUploadAudioError('再生時間を確認できませんでした。最大20分・30MBまでの音声ファイルか確認し、別の音声ファイルを選んでください。')
  } finally {
    setIsValidatingUploadAudio(false)
  }
}
...
function verifyAudioSeek(params: {
  ...
  setAudioSeekDebugById((prev) => ({ ...prev, [key]: entry }))
  window.setTimeout(() => {
    ...
    setAudioSeekDebugById((prev) => {
      ...
      return { ...prev, [key]: { ...previousEntry, afterDelay, afterRetry, ignored } }
    })
  }, 250)
}
