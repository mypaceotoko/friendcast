...
const updateDiscoverableSetting = async (nextValue: boolean) => {
  if (!session?.user?.id || !profile || !sb || isDiscoverableSaving) return
  const userId = session.user.id
  const previousValue = isProfileDiscoverable(profile)
  ...
  try {
    const { data, error } = await sb
      ...
      .single()
    if (error) throw error
    setProfile((prev) => prev ? { ...prev, is_discoverable: data?.is_discoverable ?? nextValue } : prev)
    setDiscoverUsers((prev) => prev.map((item) => item.id === userId ? { ...item, is_discoverable: data?.is_discoverable ?? nextValue } : item))
    setFriendSuggestions((prev) => prev.filter((item) => item.id !== userId))
  } catch (error) {
    const supabaseError = error as { message?: string; code?: string; details?: string; hint?: string }
    console.error('update discoverable setting failed', {
      action: 'update-discoverable-setting',
      userId,
      nextValue,
    ...
    setProfile((prev) => prev ? { ...prev, is_discoverable: previousValue } : prev)
