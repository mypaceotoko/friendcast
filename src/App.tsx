...
type ProfileTab = 'posts' | 'audio' | 'replies' | 'likes' | 'following' | 'followers'
...
const [profileFollowCounts, setProfileFollowCounts] = useState<FollowCounts>({ following: 0, followers: 0 })
...
const loadProfileFollowData = async (userId: string) => {
  ...
  try {
    ...
    const followingIdsForProfile = (followingRes.data ?? []).map((row) => row.following_id).filter(Boolean)
    const followerIdsForProfile = (followersRes.data ?? []).map((row) => row.follower_id).filter(Boolean)
    setProfileFollowCounts({ following: followingIdsForProfile.length, followers: followerIdsForProfile.length })
    const uniqueIds = Array.from(new Set([...followingIdsForProfile, ...followerIdsForProfile]))
    if (uniqueIds.length === 0) {
      setProfileFollowingUsers([])
      setProfileFollowerUsers([])
      return
    }
    const { data: profileRows, error: profileError } = await sb!.from('profiles').select('id,username,display_name,avatar_url,bio').in('id', uniqueIds)
    if (profileError) throw profileError
    const map = new Map((profileRows ?? []).map((row) => [row.id, row as Profile]))
    setProfileFollowingUsers(followingIdsForProfile.map((id) => map.get(id)).filter(Boolean) as Profile[])
    setProfileFollowerUsers(followerIdsForProfile.map((id) => map.get(id)).filter(Boolean) as Profile[])
  } catch (error) {
    console.error('load profile follow data failed', error)
    setProfileFollowListError('フォロー情報を読み込めませんでした。時間をおいて再度お試しください。')
    setProfileFollowingUsers([])
    setProfileFollowerUsers([])
  } finally {
...
const loadFriendSuggestions = async (userId: string) => {
  ...
  try {
    const { data: firstHopRows, error: firstHopError } = await sb.from('follows').select('following_id').eq('follower_id', userId)
    if (firstHopError) throw firstHopError
    const firstHopIds = Array.from(new Set((firstHopRows ?? []).map((row) => row.following_id).filter(Boolean)))
    if (!firstHopIds.length) {
      setFriendSuggestions([])
      setFriendSuggestionsStatus('loaded')
      return
    }
    const { data: secondHopRows, error: secondHopError } = await sb.from('follows').select('following_id').in('follower_id', firstHopIds)
    if (secondHopError) throw secondHopError
    const blockedIds = new Set<string>([userId, ...firstHopIds])
    const candidateIds = Array.from(new Set((secondHopRows ?? []).map((row) => row.following_id).filter((id): id is string => !!id && !blockedIds.has(id))))
    if (!candidateIds.length) {
      setFriendSuggestions([])
      setFriendSuggestionsStatus('loaded')
      return
    }
    const { data: candidateProfiles, error: candidateProfilesError } = await sb.from('profiles').select('id,username,display_name,avatar_url,bio').in('id', candidateIds.slice(0, 40))
    if (candidateProfilesError) throw candidateProfilesError
    setFriendSuggestions((candidateProfiles ?? []).slice(0, 10))
    setFriendSuggestionsStatus('loaded')
  } catch (error) {
...
useEffect(() => {
  if (screen !== 'profile' || !activeProfileId || !session?.user?.id) return
  void loadProfileFollowData(activeProfileId)
}, [screen, activeProfileId, session?.user?.id, followingIds])
...
const toggleFollow = async (targetUserId: string) => {
  ...
  try {
    if (isFollowing(targetUserId)) {
      ...
      if (activeProfileId === me) setProfileFollowCounts((prev) => ({ ...prev, following: Math.max(0, prev.following - 1) }))
      if (activeProfileId === targetUserId) setProfileFollowCounts((prev) => ({ ...prev, followers: Math.max(0, prev.followers - 1) }))
    } else {
      const { error } = await sb!.from('follows').insert({ follower_id: me, following_id: targetUserId })
      if (error) throw error
      setFollowingIds((prev) => new Set(prev).add(targetUserId))
      if (activeProfileId === me) setProfileFollowCounts((prev) => ({ ...prev, following: prev.following + 1 }))
      if (activeProfileId === targetUserId) setProfileFollowCounts((prev) => ({ ...prev, followers: prev.followers + 1 }))
    }
  } catch (error) {
