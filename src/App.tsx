...
const seekVoiceCommentBy = (commentId: string, deltaSeconds: number) => {
  ...
  const duration = Number.isFinite(audio.duration) ? audio.duration : (voiceCommentDurationById[commentId] ?? 0)
  const bounded = duration > 0 ? duration : Number.MAX_SAFE_INTEGER
  audio.currentTime = Math.max(0, Math.min(bounded, audio.currentTime + deltaSeconds))
}

const cycleVoiceCommentRate = (commentId: string) => {
  const audio = voiceCommentAudioRef.current
  if (!audio || playingVoiceCommentId !== commentId) return
  const steps = [1, 1.2, 1.5, 2]
  const current = steps.includes(audio.playbackRate) ? audio.playbackRate : 1
