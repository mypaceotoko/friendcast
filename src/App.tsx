...
const loadBookmarkedPostIds = async (userId: string) => {
  ...
  setSavedPostIds((data ?? []).map((row) => row.post_id).filter(Boolean))
}
...
const isPostBookmarked = (postId: string) => savedPostIds.includes(postId)
const toggleBookmarkPost = async (postId: string) => {
  const userId = session?.user.id
  if (!userId) return
  const wasBookmarked = isPostBookmarked(postId)
  setSavedPostIds((prev) => wasBookmarked ? prev.filter((id) => id !== postId) : [...prev, postId])
  try {
    if (wasBookmarked) {
    ...
    setSavedPostIds((prev) => wasBookmarked ? [...prev, postId] : prev.filter((id) => id !== postId))
  }
}
