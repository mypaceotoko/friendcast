...
useEffect(() => {
  ...
  const bootstrap = async () => {
    try {
      ...
      await withTimeout(loadCloseFriends(data.session.user.id), INIT_TIMEOUT_MS)
      await withTimeout(loadDiscoverUsers(data.session.user.id), INIT_TIMEOUT_MS)
  ...
  const { data: listener } = sb!.auth.onAuthStateChange(async (_event, newSession) => {
    ...
    try {
      ...
      await withTimeout(loadCloseFriends(newSession.user.id), INIT_TIMEOUT_MS)
      await withTimeout(loadDiscoverUsers(newSession.user.id), INIT_TIMEOUT_MS)
      await withTimeout(loadPosts(), INIT_TIMEOUT_MS)
    } catch (error) {
...
const homePosts = useMemo(() => {
  ...
  return posts.filter((post) => {
    ...
    if (post.visibility === 'private') return post.user_id === myId
    if (post.visibility !== 'close_friends') return true
    return post.user_id === myId || incomingCloseFriendOwnerIds.has(post.user_id)
  })
}, [posts, followingIds, incomingCloseFriendOwnerIds, session?.user.id])
