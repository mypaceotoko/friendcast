...
const useInviteCode = async (rawCode?: string) => {
  ...
  if (error || !data) { setInviteActionError('招待コードが見つかりませんでした。'); setIsInviteUsing(false); return }
  if (data.inviter_id === session.user.id) { setInviteActionError('自分の招待コードは使用できません。'); setIsInviteUsing(false); return }
  ...
  let wasAlreadyFollowing = followingIds.has(data.inviter_id)
  if (!wasAlreadyFollowing) {
    const { data: existingFollow, error: existingFollowError } = await sb
      .from('follows')
      .select('following_id')
      .eq('follower_id', session.user.id)
      .eq('following_id', data.inviter_id)
      .maybeSingle()
    if (existingFollowError) console.error('invite existing follow check failed', existingFollowError)
    wasAlreadyFollowing = !!existingFollow
  }
  ...
  const inviteSuccessMessage = wasAlreadyFollowing
    ? '招待コードを確認しました。すでにつながっている相手です。'
    : '招待コードを使いました。招待してくれた人をフォローしました。'
  setInviteActionMessage(inviteSuccessMessage)
  setInviteSuccessNotice(inviteSuccessMessage)
  setInviteCloseFriendCard({ inviterId: data.inviter_id, inviterName, inviterAvatarUrl, added: closeFriendIds.has(data.inviter_id), alreadyFollowing: wasAlreadyFollowing })
  setIsInviteUsing(false)
}
