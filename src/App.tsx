...
type ActivityItem = { id: string; type: 'comment' | 'like' | 'repost' | 'follow' | 'invite_used'; actor: Profile; postId?: string; postPreview?: string; body?: string; code?: string; createdAt: string }
...
const loadActivities = async () => {
  ...
  try {
    ...
    const [commentsRes, likesRes, repostsRes, followsRes, invitesRes] = await Promise.all([
      myPostIds.length ? sb.from('comments').select('id,post_id,user_id,body,created_at').in('post_id', myPostIds).neq('user_id', myId).order('created_at',{ascending:false}).limit(20) : Promise.resolve({ data: [], error: null }),
    ...
    if (errors.length) { console.error('activity fetch failed', errors); setActivityError('アクティビティを読み込めませんでした') }
    ...
    const items: ActivityItem[] = [
      ...(commentsRes.data ?? []).map((row) => ({ id: `comment_${row.id}`, type: 'comment' as const, actor: actors[row.user_id] ?? { ...fallbackActor, id: row.user_id }, postId: row.post_id, postPreview: toActivityPostPreview(row.post_id), body: row.body, createdAt: row.created_at })),
      ...(likesRes.data ?? []).map((row) => ({ id: `like_${row.post_id}_${row.user_id}`, type: 'like' as const, actor: actors[row.user_id] ?? { ...fallbackActor, id: row.user_id }, postId: row.post_id, postPreview: toActivityPostPreview(row.post_id), createdAt: row.created_at })),
      ...(repostsRes.data ?? []).map((row) => ({ id: `repost_${row.post_id}_${row.user_id}`, type: 'repost' as const, actor: actors[row.user_id] ?? { ...fallbackActor, id: row.user_id }, postId: row.post_id, postPreview: toActivityPostPreview(row.post_id), createdAt: row.created_at })),
      ...(followsRes.data ?? []).map((row) => ({ id: `follow_${row.follower_id}_${row.created_at}`, type: 'follow' as const, actor: actors[row.follower_id] ?? { ...fallbackActor, id: row.follower_id }, createdAt: row.created_at })),
      ...(invitesRes.data ?? []).filter((row) => !!row.used_by).map((row) => ({ id: `invite_${row.id}`, type: 'invite_used' as const, actor: actors[row.used_by as string] ?? { ...fallbackActor, id: row.used_by as string }, code: row.code, createdAt: row.used_at ?? row.id }))
...
useEffect(() => { if (isActivityOpen) void loadActivities() }, [isActivityOpen])
const activityMessage = (item: ActivityItem) => item.type === 'comment' ? 'あなたの投稿にコメントしました' : item.type === 'like' ? 'あなたの投稿にいいねしました' : item.type === 'repost' ? 'あなたの投稿をリポストしました' : item.type === 'follow' ? 'あなたをフォローしました' : '招待コードを使いました'
