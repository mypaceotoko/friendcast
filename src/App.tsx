...
useEffect(() => {
  ...
  const bootstrap = async () => {
    try {
      ...
      await withTimeout(loadIncomingCustomPosts(data.session.user.id), INIT_TIMEOUT_MS)
  ...
  const { data: listener } = sb!.auth.onAuthStateChange(async (_event, newSession) => {
    ...
    try {
      ...
      await withTimeout(loadIncomingCustomPosts(newSession.user.id), INIT_TIMEOUT_MS)
      await withTimeout(loadPosts(), INIT_TIMEOUT_MS)
    } catch (error) {
