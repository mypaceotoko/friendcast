...
const ENABLE_PROFILE_FOLLOW_STATS = false
...
const followingUsers = profileFollowingUsers ?? []
const followerUsers = profileFollowerUsers ?? []
...
const loadProfileFollowData = async (targetProfileId: string) => {
  if (!ENABLE_PROFILE_FOLLOW_STATS || !sb || !session?.user?.id || !targetProfileId) return
  ...
  try {
    ...
    if (followingErr || followerErr) throw followingErr ?? followerErr
    const followingIdsLocal = Array.from(new Set((followingRows ?? []).map((r) => r.following_id).filter(Boolean))) as string[]
    const followerIdsLocal = Array.from(new Set((followerRows ?? []).map((r) => r.follower_id).filter(Boolean))) as string[]
    const allIds = Array.from(new Set([...followingIdsLocal, ...followerIdsLocal]))
    let map: Record<string, Profile> = {}
    if (allIds.length) {
      const { data: profileRows, error: profileErr } = await sb.from('profiles').select('id,username,display_name,avatar_url,bio').in('id', allIds)
      if (profileErr) throw profileErr
      map = (profileRows ?? []).reduce<Record<string, Profile>>((acc, row) => { acc[row.id] = row as Profile; return acc }, {})
      setProfileMap((prev) => ({ ...prev, ...Object.fromEntries((profileRows ?? []).map((row) => [row.id, { username: row.username, display_name: row.display_name, avatar_url: row.avatar_url, bio: row.bio ?? '' }])) }))
    }
    setProfileFollowCounts({ following: followingIdsLocal.length, followers: followerIdsLocal.length })
    setProfileFollowingUsers(followingIdsLocal.map((id) => map[id]).filter(Boolean))
    setProfileFollowerUsers(followerIdsLocal.map((id) => map[id]).filter(Boolean))
  } catch (error) {
